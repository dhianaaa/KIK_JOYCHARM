...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: JCBottomNavBar(_currentIndex),
    );
  }
}
...
Future<void> _fetchProducts() async {
  ...
  if (result['status'] == true) {
    ...
    setState(() {
      _newProducts = allProducts
          .where((p) => p.category == 'New')
          .toList();

      _recommendProducts = allProducts
          .where((p) => p.category == 'Rekomendasi')
          .toList();
  ...
  SliverToBoxAdapter(
    child: _isLoading
        ...
        : _buildApiProductRow(_newProducts),
  ),
  ...
  SliverToBoxAdapter(
    child: _isLoading
        ? const SizedBox()
        : _buildApiProductRow(_recommendProducts),
  ),
...
Widget _buildApiProductRow(List<ProductModel> products) {
  return SizedBox(
    height: 240,
    child: ListView.builder(
      scrollDirection: Axis.horizontal,
